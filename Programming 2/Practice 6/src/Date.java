...
public class Date {
	...
	// -----------------------------------------------------------------
	// This method compares components of two dates and returns true
	// if all components of both dates match exactly
	public boolean equals(Date secondDate) {
		if (this.month == secondDate.month && this.day == secondDate.day && this.year == secondDate.year)
			return true;
		else
			return false;
	...
	// -----------------------------------------------------------------
	// This method compares components of two dates and returns 1
	// if the first date is later than the second, zero if the dates
	// are equal, and -1 if the first date is earlier than the
	// first.
	public int compareTo(Date secondDate) {
		int returnValue = 0;

		if (this.equals(secondDate))
			returnValue = 0;
		else {
			// Initiatate test for less than
			if (this.year < secondDate.year)
				returnValue = -1;
			else if (year == secondDate.year && month < secondDate.month)
				returnValue = -1;
			else if (month == secondDate.month && day < secondDate.day)
				returnValue = -1;
			else
				returnValue = 1; // Otherwise, comparision must be greater than
		}
		return returnValue;
