...
public class ContactList {
	 ...
	 * @param month,
	 *            day of birth date
	 * @return String with all all data related to all contacts with matching birth
	 *         dates.
	 */
	...
	/**
	 * Linear search for a contact by zip code
	 *
	 * @param zipCode
	 * @return String with all all data related to all contacts with matching
	 *         zipCode.
	 */
	public String searchContactsByZipCode(int zipCode) {
		String outputString = "";

		for (int i = 0; i < numElems; i++) {
			if (myContacts[i].matchesAreaCode(zipCode))
				outputString += myContacts[i].toString();
		}
		return outputString;
	}
