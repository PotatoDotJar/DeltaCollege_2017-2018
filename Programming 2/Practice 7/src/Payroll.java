...
public class Payroll {
	...
	public final String FILENAME = "inpay2.txt";
	...
	public Payroll() {
		...
		try {
			...
			while (inputFileScanner.hasNext()) {
				...
				// Piece Worker - create object and add to list
				if (code.equals("P")) {
					int completedWork = inputFileScanner.nextInt();
					PieceWorker pieceWorker = new PieceWorker(name, id, completedWork);
					System.out.println("Added " + name + " " + id + " " + completedWork);
					workerList[i] = pieceWorker;
				}
				i++;
	...
	public void preparePayroll() {
		for (int i = 0; i < numElems; i++) {
			workerList[i].calcGrossPay();
			workerList[i].calcTaxNet();
		}
	}
	...
	public String toString() {
		...
		for (int i = 0; i < numElems; i++) {
			...
			outString += String.format("%d8", workerList[i].getID());
