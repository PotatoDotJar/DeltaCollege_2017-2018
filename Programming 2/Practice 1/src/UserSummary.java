...
public class UserSummary {
	...
	public static void main(String args[]) throws IOException {
		...
		int minTrans = 0; // To retain min transactons value
		...
		double aveTransPerDay = (double) totalTrans / 7.0D;
		double aveTransPerUser = (double) totalTrans / totalUsers;
		double aveTransPerDayPerUser = aveTransPerDay / aveTransPerUser;
		...
		if (numElems > 0) {
			...
			outMessage += String.format("Min transactions on: " + minTransDay) + "\n";
			outMessage += String.format("Ave. Transactions/day/user: %5.1f", aveTransPerDayPerUser) + "\n";
			outMessage += String.format("Ave. Transactions/user: %4.1f", aveTransPerUser);
		} else
			outMessage = "ERROR:  No data processed";

		// Display the results
		JOptionPane.showMessageDialog(null, outMessage, "Statistical Summary", JOptionPane.INFORMATION_MESSAGE);
