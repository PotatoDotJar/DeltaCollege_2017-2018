...
public class OrderList {
	 ...
	 * @param state
	 *            enumerated type defining current state of list
	 ...
	 * 
	 * @param newElement
	 *            Element to be added to the list in proper position.
	...
	/**
	 * Delete existing element into the list. List ordering is preserved. If element
	 * is not in list, nothing is done.
	 * 
	 * @param oldElement
	 *            to be deleted.
	 */
	...
	public int search(String target) {
		...
		while (!found && first <= last) {
			...
			if (data[middle].equals(target)) // If value is found at mid
			{
				found = true;
				position = middle;
			} else if ((sortStatus == SortState.ASCENDING && data[middle].compareTo(target) > 0)
					|| (sortStatus == SortState.DESCENDING && data[middle].compareTo(target) < 0))
				last = middle - 1;
			else
	...
	/**
	 * Perform bubble sort on list either ascending or descending as indicated by
	 * parameter.
	 */
	public void sortList(SortState desiredState) {
		...
		for (lastPos = numElems - 1; lastPos >= 0; lastPos--) {
			for (index = 0; index <= lastPos - 1; index++) {
				...
				if ((sortStatus == SortState.ASCENDING && data[index].compareTo(data[index + 1]) > 0)
						|| (sortStatus == SortState.DESCENDING && data[index].compareTo(data[index + 1]) < 0)) {
	 ...
	 * 
	 * @return true if lists are equal in all respects; false otherwise.
	 */
	public boolean equals(OrderList secondList) {

		if (this.numElems == secondList.numElems) {
			for (int i = 0; i < this.numElems; i++) {
				if (!this.data[i].trim().equals(secondList.data[i].trim())) {
					return false;
				}
			}
			return true;
		} else {
			return false; // Return outcome
		}
