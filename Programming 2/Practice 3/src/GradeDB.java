...
public class GradeDB {
	...
	private final String INPUT_FILENAME = "gradeData2.txt";
	...
	public GradeDB() {
		...
		try {
			...
			absences = new int[MAX_ARRAY_SIZE];
			...
			while (inputFileScanner.hasNext()) {
				...
				absences[i] = Integer.parseInt(dataTokens.nextToken());
	...
	/**
	 * Sort all data by student number of Absences - Descending
	 */
	public void sortAbsenceAsc() {
		for (int lastPos = numElems - 1; lastPos >= 0; lastPos--)
			for (int index = 0; index <= lastPos - 1; index++)
				if (absences[index] < absences[index + 1])
					swapStudents(index, index + 1);
	...
	public void swapStudents(int index1, int index2) {
		...
		int tempAbsence = absences[index1];
		absences[index1] = absences[index2];
		absences[index2] = tempAbsence;
	}
	...
	public String getStudentFormatted(int index) {
		String outString = id[index] + "\t" + name[index] + "\t" + grade[index] + "\t" + gpa[index] + "\t"
				+ absences[index];
	...
	public String toString() {
		String outString = "ID\tName\tGrade\tGPA\tAbsences\n";
